...
public class GoGenerate {
	...
	public static void main(String[] args) {
		...
		for(String cassConfig: args){
			try {
				KeySpaceContext ksc = reader.read("src\\cassandra\\resources\\"+cassConfig);
				initCassandra(ksc);
			...
			if(ksc.getCfs().containsKey(cfd.getName())){//只处理配置cf
				...
				if(StringUtils.isNotBlank(content)){
					...
					try {
						...
						genFtl(ftlClassInfo, targetSource, "CassandraDAO.ftl", daoDir, "Dao");
						File daoImplDir = getDirectory(projectName, targetSource, targetPackage, "dao.impl");
						genFtl(ftlClassInfo, targetSource, "CassandraDAOImpl.ftl", daoImplDir, "DaoImpl");
					} catch (IOException e) {
						e.printStackTrace();
					}							
				}
	...
	public static void genFtl(FtlClassInfo clazzinfo, String targetSource,
		...
		try {
			...
			cfg.setDirectoryForTemplateLoading(new File("src/cassandra/resources"));
